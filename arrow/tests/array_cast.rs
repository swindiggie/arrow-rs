...
fn make_large_list_array() -> LargeListArray {
    ...
    // Construct a list array from the above two
    let list_data_type = DataType::LargeList(Arc::new(Field::new("element", DataType::Int32, true)));
    let list_data = ArrayData::builder(list_data_type)
        .len(3)
        .add_buffer(value_offsets)
        .add_child_data(value_data)
...
fn get_all_types() -> Vec<DataType> {
    ...
    let mut types = vec![
        ...
        Utf8,
        LargeUtf8,
        List(Arc::new(Field::new("element", DataType::Int8, true))),
        List(Arc::new(Field::new("element", DataType::Utf8, true))),
        FixedSizeList(Arc::new(Field::new("element", DataType::Int8, true)), 10),
        FixedSizeList(Arc::new(Field::new("element", DataType::Utf8, false)), 10),
        LargeList(Arc::new(Field::new("element", DataType::Int8, true))),
        LargeList(Arc::new(Field::new("element", DataType::Utf8, false))),
